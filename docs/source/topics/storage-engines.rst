...
Every time you login to Telegram, some personal piece of data are created and held by both parties (the client, PyroRatnaGram
...
authorization process from scratch each time, PyroRatnaGram needs to store the generated session data somewhere.

Different Storage Engines
-------------------------

PyroRatnaGram offers two different types of storage engines: a **File Storage** and a **Memory Storage**.
...
``my_account.session``. Any subsequent client restart will make PyroRatnaGram search for a file named that way and the
...
restart will make PyroRatnaGram search for a database named that way and the session database will be automatically loaded.
...
Session strings are useful when you want to run authorized PyroRatnaGram clients on platforms where their ephemeral
